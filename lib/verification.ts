...
// This prevents the "supabaseUrl is required" crash during Next.js build
// when env vars aren't available at compile time.
...
export async function checkEmailDeliverability(email?: string): Promise<CheckResult> {
  ...
  if (!process.env.KICKBOX_API_KEY) {
    throw new Error("KICKBOX_API_KEY is not set");
  }
  const res = await fetch(
    `https://api.kickbox.com/v2/verify?email=${encodeURIComponent(email)}&apikey=${process.env.KICKBOX_API_KEY}`
  );
  if (!res.ok) throw new Error(`Kickbox API error: ${res.status} ${res.statusText}`);
...
export async function checkPhoneLineType(phone?: string): Promise<CheckResult> {
  ...
  if (!res.ok) throw new Error(`Twilio Lookup API error: ${res.status} ${res.statusText}`);
  const data = await res.json();
...
export async function checkHLR(phone?: string): Promise<CheckResult> {
  ...
  if (!res.ok) throw new Error(`IPQS API error: ${res.status} ${res.statusText}`);
  const data = await res.json();
  ...
  else if (data.active === false) hlrStatus = "unreachable";
  else hlrStatus = "unknown";
  let weight = 0;
...
export async function checkSuppressionList(email?: string, phone?: string): Promise<CheckResult> {
...
export async function checkDuplicates(email?: string, phone?: string, clientId?: string): Promise<CheckResult> {
